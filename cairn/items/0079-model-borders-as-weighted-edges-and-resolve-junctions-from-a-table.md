...
- [ ] Every combination the Unicode box-drawing block can express maps to a glyph, and the ones it cannot are documented with the chosen approximation
- [ ] Merging is commutative and associative, proven over generated input
- [ ] Mixed weights resolve to the heavier glyph where Unicode has one (`┝`, `╪`), and to a documented substitute where it does not
- [ ] The `ascii` set resolves the same keys to `+ - |`, so geometry is identical across sets
